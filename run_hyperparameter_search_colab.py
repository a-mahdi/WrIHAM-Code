#!/usr/bin/env python3
"""
Arabic Writer Identification - Hyperparameter Search (GOOGLE COLAB VERSION)
Bayesian Optimization using Optuna - Optimized for Colab with A100 GPU

Task: Find optimal hyperparameters for both ConvNeXt and Transformer models
Data: Historical Arabic manuscript line images (stored on Google Drive)
Optimization: Bayesian search with early stopping

✨ COLAB FEATURES:
- Single GPU optimization (perfect for Colab A100)
- Google Drive integration (automatic mounting)
- Quick test mode (verify setup in 10-20 minutes)
- Progress tracking optimized for notebooks
...
def setup_colab_environment():
    ...
    Checks if running in Colab and provides Drive mounting instructions.
    """
    try:
        import google.colab
        IN_COLAB = True
...
# ============================================================
# MAIN ENTRY POINT (COLAB VERSION - SINGLE GPU ONLY)
# ============================================================

def main():
    """Main entry point for Colab hyperparameter search (Single GPU)."""

    # Check Colab environment is ready
    if not COLAB_READY:
        print("\n❌ Environment not ready. Please mount Google Drive first.")
        sys.exit(1)

    # Parse arguments
    parser = argparse.ArgumentParser(
        description='Arabic Writer Identification - Hyperparameter Search (GOOGLE COLAB VERSION)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
🌟 COLAB VERSION - Optimized for Single GPU (A100)

Example - Quick Test (RECOMMENDED FIRST):
  !python run_hyperparameter_search_colab.py \\
    --data_root /content/drive/MyDrive/Mirath_extracted_lines \\
    --checkpoint_dir /content/drive/MyDrive/checkpoints \\
    --quick_test
...
💡 All data and results are saved to Google Drive automatically!
        """
    )
    parser.add_argument('--data_root', type=str, required=True,
                   help='Path to Mirath_extracted_lines on Google Drive')
    parser.add_argument('--checkpoint_dir', type=str, required=True,
                   help='Directory on Google Drive to save checkpoints')
    ...
    parser.add_argument('--use_all_writers', action='store_true',
                   help='Use all writers (21 writers)')
    ...
    if num_gpus == 0:
        ...
        print("   Colab should have GPU enabled. Go to: Runtime → Change runtime type → GPU")
        sys.exit(1)

    # Display GPU info
    print(f"\n{'='*70}")
    print("🚀 GOOGLE COLAB - SINGLE GPU MODE")
    print(f"{'='*70}")
    ...
    print("💾 Note: All results will be saved to Google Drive automatically!\n")
