#!/usr/bin/env python3
"""
Arabic Writer Identification - Hyperparameter Search with Multi-GPU
Bayesian Optimization using Optuna with DistributedDataParallel

Task: Find optimal hyperparameters for both ConvNeXt and Transformer models
Data: Historical Arabic manuscript line images
Optimization: 100 trials with early stopping
Multi-GPU: 4 GPUs with DistributedDataParallel
...
# ============================================================
# SECTION 1: IMPORTS & MULTI-GPU SETUP
# ============================================================
...
class TrialConfig(BaseConfig):
    ...
    def __init__(self, hyperparams, rank=0):
        super().__init__()
        
        # Store rank for distributed training
        self.rank = rank
        ...
        # Adjust batch size for DDP (split across GPUs)
        if self.USE_DDP:
            self.BATCH_SIZE_PER_GPU = self.BATCH_SIZE // torch.cuda.device_count()
        else:
            self.BATCH_SIZE_PER_GPU = self.BATCH_SIZE
...
print("✅ Section 1: Imports and Multi-GPU setup complete")
...
class WriterBalancedSampler(Sampler):
    ...
    def print_sampler_info(self, writers, rank=0):
...
class DistributedSamplerWrapper(Sampler):
    """
    Wrapper for writer-balanced sampler to work with DistributedDataParallel.
    
    Splits the balanced samples across multiple GPUs.
    
    Args:
        sampler: WriterBalancedSampler instance
        num_replicas: Number of processes (GPUs)
        rank: Process rank (GPU id)
        shuffle: Whether to shuffle (should be False, sampler already randomizes)
    """
    
    def __init__(self, sampler, num_replicas=None, rank=None, shuffle=False):
        ...
        self.num_samples = len(self.sampler) // self.num_replicas
        self.total_size = self.num_samples * self.num_replicas
    
    def __iter__(self):
        """Get indices for this GPU."""
        # Get all indices from base sampler
...
def create_model(config, rank=0):
    """
    ...
    """
    print_rank_0(f"\n{'='*70}", rank)
    print_rank_0(f"CREATING MODEL: {config.MODEL_TYPE.upper()}", rank)
    print_rank_0(f"{'='*70}", rank)
    ...
    print_rank_0(f"\n📊 Model Statistics:", rank)
    print_rank_0(f"  Total parameters:      {total_params:,}", rank)
    print_rank_0(f"  Trainable parameters:  {trainable_params:,}", rank)
    print_rank_0(f"  Frozen parameters:     {frozen_params:,}", rank)
    print_rank_0(f"  Embedding dimension:   {config.EMBEDDING_DIM}", rank)
    print_rank_0(f"  Number of classes:     {config.NUM_CLASSES}", rank)
    print_rank_0(f"{'='*70}\n", rank)
...
def gather_metrics(metrics_dict, world_size, rank):
    """
    ...
    for key, value in metrics_dict.items():
        if isinstance(value, (int, float)):
            ...
            gathered[key] = gather_metrics(value, world_size, rank)
        else:
            gathered[key] = value
...
class CheckpointManager:
    ...
    def save_trial_summary(self, config, metrics, history, rank=0):
        """
...
# ============================================================
# MAIN DISTRIBUTED WORKER
# ============================================================

def distributed_worker(rank, world_size, config_base, n_trials):
    """
    Distributed training worker for each GPU.
    ...
    # Build data index once (rank 0 only, then broadcast)
    if rank == 0:
    ...
    # Broadcast data index to all ranks
    if world_size > 1:
        # Use pickle to broadcast complex objects
        if rank == 0:
...
def main():
    ...
    # Parse arguments
    parser = argparse.ArgumentParser(description='Arabic Writer Identification - Hyperparameter Search')
    ...
    if world_size > 1:
        ...
        mp.spawn(
            distributed_worker,
            args=(world_size, config_base, args.n_trials),
            nprocs=world_size,
            join=True
        )
    else:
        # Single GPU
        distributed_worker(0, 1, config_base, args.n_trials)
...
print("\n" + "="*70)
print("HYPERPARAMETER SEARCH SCRIPT READY")
print("="*70)
print("\nUsage:")
...
class WriterBalancedSampler(Sampler):
    ...
    Algorithm:
    1. Cap each page at L lines per epoch (from config)
    2. Calculate per-writer quota: Q_w = min(Q_target, P_w * L, r_max * N_w)
    3. Without-replacement cycles: track exposure per line
    4. Round-robin across writers until each hits quota
    
    This prevents overfitting by:
    - Balancing writer representation
    - Limiting exposure to same lines
    - Ensuring all writers get fair sampling
    
    All parameters come from config (trial-specific):
    - LINES_PER_PAGE_CAP (L)
    - QUOTA_TARGET (Q_target)
    - R_MAX (r_max)
    """
    
    def __init__(self, lines, pages, config):
        """
        Args:
            lines: List of line dictionaries
            pages: Dict mapping page_id -> [line_indices]
            config: Configuration with LINES_PER_PAGE_CAP, QUOTA_TARGET, R_MAX
        """
        self.lines = lines
        self.pages = pages
        self.config = config
        ...
        self.L = config.LINES_PER_PAGE_CAP
        self.Q_target = config.QUOTA_TARGET
        ...
        self.writer_quotas = {}
        for w in self.writers:
            N_w = len(self.writer_lines[w])  # Total lines for writer
            P_w = len(self.writer_pages[w])  # Total pages for writer
            Q_w = int(min(self.Q_target, P_w * self.L, self.r_max * N_w))
            self.writer_quotas[w] = Q_w
        
        # Initialize exposure tracking (0 = never seen)
        self.line_exposure = {idx: 0 for idx in range(len(lines))}
        
        # Print sampler info (only on rank 0)
        if not hasattr(config, 'rank') or config.rank == 0:
            print(f"\n📊 Writer-Balanced Sampler initialized:")
            print(f"  L (per-page cap): {self.L}")
            print(f"  Q_target: {self.Q_target}")
            ...
            for w in self.writers:
                ...
                P_w = len(self.writer_pages[w])
                Q_w = self.writer_quotas[w]
                print(f"    Writer {w}: {P_w:3d} pages, {N_w:5d} lines → quota {Q_w:5d}")
    ...
    def __iter__(self):
        ...
        for w in self.writers:
            ...
            # Cycle through pages until we hit quota
            page_idx = 0
            while len(writer_epoch_indices) < quota:
                if page_idx >= len(writer_pages):
                    # Wrapped around all pages, start over
                    page_idx = 0
                
                page_id = writer_pages[page_idx]
                page_line_indices = self.pages[page_id]
                
                # Get lines with lowest exposure (fairness)
                lines_with_exposure = [(idx, self.line_exposure[idx]) for idx in page_line_indices]
                lines_with_exposure.sort(key=lambda x: x[1])  # Sort by exposure
                
                # Take up to L lines
                num_to_take = min(self.L, len(lines_with_exposure), quota - len(writer_epoch_indices))
                selected = [idx for idx, _ in lines_with_exposure[:num_to_take]]
        ...
        # Shuffle final indices for batching
        random.shuffle(epoch_indices)
    ...
    def __len__(self):
        """Total samples per epoch."""
        return sum(self.writer_quotas.values())
    ...
    def get_coverage_report(self, writers):
        ...
        Returns:
            dict: {writer_idx: {'total_lines', 'seen_lines', 'coverage_pct', 'avg_exposure'}}
        """
        coverage = {}
        ...
        for w in self.writers:
            ...
            coverage[w] = {
                ...
                'unseen_lines': total_lines - seen_lines,
                'coverage_pct': coverage_pct,
                'avg_exposure': np.mean([self.line_exposure[idx] for idx in writer_line_indices])
            }
...
def discover_writers(data_root):
    """
    Automatically discover writer names from train directory.
    ...
    writers = sorted([d.name for d in train_path.iterdir() if d.is_dir()])
    
    if len(writers) == 0:
        raise ValueError(f"No writer directories found in {train_path}")
    
    return writers


def build_index(data_root, splits=['train', 'val'], selected_writers=None, rank=0):
    """
    Build in-memory index of all line images.
    ...
    A "page folder" is a leaf directory containing line images.
    """
    data_root = Path(data_root)
    
    # Discover writers automatically
    if selected_writers:
    ...
    for split in splits:
        ...
        writer_stats = defaultdict(lambda: {
            'pages': 0,
            'lines': 0,
            'zero_pages': 0,
        ...
        iterator = tqdm(writers, desc=f"Indexing {split}") if rank == 0 else writers
        ...
        for writer in iterator:
            ...
            for page_folder in writer_path.rglob('*'):
                ...
                if len(subdirs) > 0:
                    # Container folder (has subdirectories) - skip
                    skipped_containers += 1
                    writer_stats[writer]['containers_skipped'] += 1
                    continue
                
                # This is a leaf directory - potential page folder
                line_images = list(page_folder.glob('*.png'))
                
                if len(line_images) == 0:
                    # Empty page folder
                    zero_line_pages += 1
                    writer_stats[writer]['zero_pages'] += 1
                    continue
                
                # Valid page with line images
                page_id = f"{writer}_{page_folder.relative_to(writer_path)}".replace('/', '_').replace('\\', '_')
                writer_stats[writer]['pages'] += 1
        ...
        split_data[split] = {
            ...
            'pages': dict(pages),
            'stats': dict(writer_stats),
            'zero_line_pages': zero_line_pages,
        ...
        if rank == 0:
            print(f"\n{split.upper()} Statistics:")
            print(f"  Total lines: {len(lines):,}")
            print(f"  Total pages (with images): {len(pages):,}")
            print(f"  Zero-line pages: {zero_line_pages}")
            print(f"  Container folders skipped: {skipped_containers}")
            print(f"\n  Per-writer breakdown:")
            for writer in writers:
                stats = writer_stats[writer]
                if stats['pages'] > 0 or stats['lines'] > 0:
                    print(f"    {writer:25s}: "
                          f"{stats['pages']:3d} pages, "
                          f"{stats['lines']:5d} lines, "
                          f"{stats['zero_pages']:3d} empty")
...
def preprocess_line_image(img_path, target_height=160, target_width=1760,
                          min_ink_ratio=0, min_resized_height=0,
                          apply_binarize=False, is_train=False, config=None):
    """
    Arabic-aware preprocessing with augmentation.
    ...
    try:
        ...
        # Rotate if portrait (H > W * 1.5)
        if h > w * 1.5:
            img = cv2.rotate(img, cv2.ROTATE_90_CLOCKWISE)
            h, w = img.shape[:2]
        
        # Resize to fit inside target dimensions (preserve aspect ratio)
        scale = min(target_height / h, target_width / w)
        new_h = int(h * scale)
        new_w = int(w * scale)
        
        # Filter: check resized height
        if new_h < min_resized_height:
            return None
        
        img_resized = cv2.resize(img, (new_w, new_h), interpolation=cv2.INTER_AREA)
        
        # ========== TRAINING AUGMENTATIONS ==========
        if is_train and config is not None:
            # 1. Slight rotation (±2 degrees)
            if random.random() < 0.3:
                angle = random.uniform(-config.AUG_ROTATION_DEGREES, 
                                      config.AUG_ROTATION_DEGREES)
                center = (new_w // 2, new_h // 2)
                M = cv2.getRotationMatrix2D(center, angle, 1.0)
                img_resized = cv2.warpAffine(img_resized, M, (new_w, new_h),
                                            borderMode=cv2.BORDER_REPLICATE)
            ...
            if random.random() < 0.3:
                alpha = 1.0 + random.uniform(-config.AUG_BRIGHTNESS_RANGE, 
                                             config.AUG_BRIGHTNESS_RANGE)  # contrast
                beta = random.uniform(-10, 10)  # brightness
                img_resized = cv2.convertScaleAbs(img_resized, alpha=alpha, beta=beta)
            
            # 4. Gaussian noise
            if random.random() < 0.2:
                noise = np.random.normal(0, config.AUG_GAUSSIAN_NOISE_STD * 255, 
                                        img_resized.shape).astype(np.float32)
                img_resized = np.clip(img_resized.astype(np.float32) + noise, 
                                     0, 255).astype(np.uint8)
        
        # Binarization (if requested)
        if apply_binarize:
            gray = cv2.cvtColor(img_resized, cv2.COLOR_RGB2GRAY)
            _, binary = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
            img_resized = cv2.cvtColor(binary, cv2.COLOR_GRAY2RGB)
        ...
        # ImageNet normalization
        normalize = transforms.Normalize(
            mean=config.IMAGENET_MEAN if config else [0.485, 0.456, 0.406],
            std=config.IMAGENET_STD if config else [0.229, 0.224, 0.225]
        )
        tensor = normalize(tensor)
...
class ArabicLineDataset(Dataset):
    ...
    def __init__(self, lines, config, is_train=False):
        """
        Args:
            lines: list of dicts with 'path', 'writer_idx', 'page_id'
            config: configuration object
    ...
    def __getitem__(self, idx):
        ...
        # Determine if we apply binarization (50% in training)
        apply_binarize = self.is_train and (random.random() < self.config.AUG_BINARIZE_PROB)
        ...
        img_tensor = preprocess_line_image(
            ...
            min_ink_ratio=self.config.MIN_INK_RATIO,
            min_resized_height=self.config.MIN_RESIZED_HEIGHT,
            apply_binarize=apply_binarize,
            is_train=self.is_train,
            config=self.config
        )
...
class WriterBalancedSampler(Sampler):
    ...
    Algorithm:
    1. Cap each page at L lines per epoch
    2. Calculate per-writer quota: Q_w = min(Q_target, P_w * L, r_max * N_w)
    3. Track exposure per line (prioritize unseen lines)
    4. Round-robin across writers until each hits quota
    """
    
    def __init__(self, lines, pages, L=50, Q_target=300, r_max=2.0):
        """
        Args:
            lines: list of line dicts
            pages: dict page_id -> [line_indices]
            L: lines per page cap
            Q_target: target quota per writer
            r_max: max oversampling ratio
        """
        self.lines = lines
        self.pages = pages
        self.L = L
        ...
        # Exposure tracking (0 = never seen)
        self.line_exposure = {idx: 0 for idx in range(len(lines))}
    
    def __iter__(self):
        """Generate epoch indices with balanced sampling."""
        epoch_indices = []
        
        for w in self.writers:
            quota = self.writer_quotas[w]
            writer_pages = list(self.writer_pages[w])
            random.shuffle(writer_pages)
            ...
            while len(writer_epoch_indices) < quota:
                ...
                # Sort by exposure (prioritize unseen lines)
                lines_with_exposure = [(idx, self.line_exposure[idx]) 
                                      for idx in page_line_indices]
                lines_with_exposure.sort(key=lambda x: x[1])
                
                # Take up to L lines
                num_to_take = min(self.L, len(lines_with_exposure), 
                                 quota - len(writer_epoch_indices))
                selected = [idx for idx, _ in lines_with_exposure[:num_to_take]]
...
print("✅ Section 2: Data indexing, preprocessing, and dataset classes complete")
